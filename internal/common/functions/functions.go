...
func TotalAmountValues() ([][2]string, int, int, string, error) {
	values, err := total_amount_db.ViewTotalAmount()
	if err != nil {
		return [][2]string{}, 0, 0, "", err
	}
	...
	if !ok1 || !ok2 || !ok3 {
		return [][2]string{}, 0, 0, "", errors.New("unable to convert to int or string")
	}

	return includedCategory, totalAmount, spentAmount, status, nil
}
